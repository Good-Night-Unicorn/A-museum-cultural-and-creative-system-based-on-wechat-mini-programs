			...
			<view :style='{"minHeight":"100vh","width":"100%","padding":"20rpx 20rpx 240rpx","position":"relative","background":"#DCF7FF","height":"auto"}'>
				<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"20rpx 0 0","background":"none","display":"flex","height":"auto"}'>
					<view :style='{"margin":"0 12rpx 0 0","flex":"1","position":"relative"}' class="search-form round">
						...
						<input placeholder-style="color: #333" :style='{"border":"2rpx solid rgb(55, 232, 220)","padding":"12rpx 20rpx 12rpx 80rpx","color":"#333","borderRadius":"10rpx","background":"#fff","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.huodongmingcheng" type="text" placeholder="活动名称" ></input>
					</view>
				...
				<view :style='{"width":"100%","background":"none","height":"auto"}'>
					...
					<view class="uni-product-list" :style='{"padding":"40rpx 0 0","alignItems":"flex-start","flexWrap":"wrap","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
						<view @tap="onDetailTap(product)" class="uni-product" :style='{"padding":"0 0 20rpx","margin":"0 0 40rpx","backgroundColor":"#fff","borderRadius":"20rpx","flexWrap":"wrap","display":"flex","width":"48%","height":"auto"}' v-for="(product,index) in list" :key="index">
							<view class="uni-product-title" :style='{"padding":"4rpx 20rpx","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"1.4","fontSize":"28rpx","textOverflow":"ellipsis"}'>{{product.huodongmingcheng}}</view>
							...
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('wenchuanghuodong','修改')) || (!userid && isAuthFront('wenchuanghuodong','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('wenchuanghuodong','删除')) || (!userid && isAuthFront('wenchuanghuodong','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('wenchuanghuodong','修改')) || (!userid && isAuthFront('wenchuanghuodong','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('wenchuanghuodong','删除')) || (!userid && isAuthFront('wenchuanghuodong','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>删除</text>
								</view>
							</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`wenchuanghuodong`, params);
                } else {
                    res = await this.$api.list(`wenchuanghuodong`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('wenchuanghuodong', JSON.stringify([id]));
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`wenchuanghuodong`, searchForm);
                } else {
                    res = await this.$api.list(`wenchuanghuodong`, searchForm);
                }
				// 如果是第一页数据置空
