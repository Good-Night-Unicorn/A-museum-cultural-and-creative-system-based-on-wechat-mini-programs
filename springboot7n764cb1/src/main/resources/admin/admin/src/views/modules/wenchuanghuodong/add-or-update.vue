	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-else-if="ruleForm.huodongbianhao" label="活动编号" prop="huodongbianhao" >
					<el-input v-model="ruleForm.huodongbianhao" placeholder="活动编号" readonly></el-input>
				</el-form-item>
				...
				<el-form-item class="select" v-if="type!='info'"  label="活动状态" prop="huodongzhuangtai" >
				...
				<el-form-item v-else class="input" label="活动状态" prop="huodongzhuangtai" >
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.huodongneirong = this.ruleForm.huodongneirong.replace(reg,'../../../springboot7n764cb1/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `wenchuanghuodong/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.wenchuanghuodongCrossAddOrUpdateFlag = false;
											this.parent.search();
