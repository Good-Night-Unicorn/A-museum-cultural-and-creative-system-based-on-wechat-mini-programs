	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-else-if="ruleForm.chanpinbianhao" label="产品编号" prop="chanpinbianhao" >
					<el-input v-model="ruleForm.chanpinbianhao" placeholder="产品编号" readonly></el-input>
				</el-form-item>
				...
				<el-form-item v-else class="input" label="产品分类" prop="chanpinfenlei" >
				...
				<el-form-item class="input" v-else-if="ruleForm.shangjiariqi" label="上架日期" prop="shangjiariqi" >
					<el-input v-model="ruleForm.shangjiariqi" placeholder="上架日期" readonly></el-input>
				</el-form-item>
			</template>
			<el-form-item v-if="type!='info'"  label="民族文化" prop="minzuwenhua" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.minzuwenhua" 
					class="editor" 
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.minzuwenhua" label="民族文化" prop="minzuwenhua" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.minzuwenhua"></span>
			</el-form-item>
			...
			<el-form-item v-else-if="ruleForm.chanpinjieshao" label="产品介绍" prop="chanpinjieshao" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.chanpinjieshao"></span>
			</el-form-item>
			<el-form-item v-if="type!='info'"  label="传统故事" prop="chuantonggushi" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.chuantonggushi" 
					class="editor" 
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.chuantonggushi" label="传统故事" prop="chuantonggushi" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.chuantonggushi"></span>
			</el-form-item>
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.minzuwenhua = this.ruleForm.minzuwenhua.replace(reg,'../../../springboot7n764cb1/upload');
						this.ruleForm.chanpinjieshao = this.ruleForm.chanpinjieshao.replace(reg,'../../../springboot7n764cb1/upload');
						this.ruleForm.chuantonggushi = this.ruleForm.chuantonggushi.replace(reg,'../../../springboot7n764cb1/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `wenchuangshangpin/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.wenchuangshangpinCrossAddOrUpdateFlag = false;
											this.parent.search();
