<template>
	<div class="addEdit-block">
		...
		>
			...
			<el-form-item class="textarea" v-if="type!='info'" label="交换条件" prop="jiaohuantiaojian" >
				<el-input
					style="min-width: 200px; max-width: 600px;"
					type="textarea"
					:rows="8"
					placeholder="交换条件"
					v-model="ruleForm.jiaohuantiaojian" >
				</el-input>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.jiaohuantiaojian" label="交换条件" prop="jiaohuantiaojian" >
				<span class="text">{{ruleForm.jiaohuantiaojian}}</span>
			</el-form-item>
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `wenchuangjiaohuan/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.wenchuangjiaohuanCrossAddOrUpdateFlag = false;
											this.parent.search();
