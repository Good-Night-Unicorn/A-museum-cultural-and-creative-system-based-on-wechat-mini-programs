	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="upload" v-if="type!='info'&& !ro.yuyinjiangjie" label="语音讲解" prop="yuyinjiangjie" >
					<file-upload
						tip="点击上传语音讲解"
						action="file/upload"
						:limit="1"
						:type="3"
						:multiple="true"
						:fileUrls="ruleForm.yuyinjiangjie?ruleForm.yuyinjiangjie:''"
						@change="yuyinjiangjieUploadChange"
					></file-upload>
				</el-form-item>
				<el-form-item v-else-if="ruleForm.yuyinjiangjie" label="语音讲解" prop="yuyinjiangjie" >
					<el-button class="viewBtn" type="text" size="small" @click="download($base.url+ruleForm.yuyinjiangjie)">
	...
	export default {
		...
		methods: {
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.xiangqing = this.ruleForm.xiangqing.replace(reg,'../../../springboot7n764cb1/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					if(this.ruleForm.yuyinjiangjie!=null) {
						this.ruleForm.yuyinjiangjie = this.ruleForm.yuyinjiangjie.replace(new RegExp(this.$base.url,"g"),"");
					}
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `yuyinjiangjie/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.yuyinjiangjieCrossAddOrUpdateFlag = false;
											this.parent.search();
			...
			yuyinjiangjieUploadChange(fileUrls) {
				this.ruleForm.yuyinjiangjie = fileUrls;
			},
		}
	};
</script>
